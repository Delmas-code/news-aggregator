...
async def get_webhooks(db: AsyncSession, skip: int = 0, limit: int =10, field : str = None, value: str = None):
    result = await db.execute(select(Webhook).offset(skip).limit(limit))
    return result.scalars().all()


# Function to filter by given fields if 
async def get_filtered_webhooks(db: AsyncSession,  field: str, value, skip: int=0, limit: int = 10):
    try:
...
async def get_specific_column(db: AsyncSession, field, limit: int = None, skip : int = 0, cond_field = None, cond_value = None):
    ...
    if cond_field and cond_value:
        stmt = select(filter_column).where(getattr(Webhook, cond_field) == cond_value).offset(skip).limit(limit)   
    else:
...
async def create_webhook(db: AsyncSession, webhook: WebhookCreate):
    ...
    """ Hash the secret"""
    db_webhook.set_secret(webhook.secret)
    db.add(db_webhook)
    await db.commit()
    await db.close()
...
async def update_webhook(db: AsyncSession, webhook_id: str, webhook: WebhookUpdate):
...
async def delete_webhook(db: AsyncSession, webhook_id: str):
    db_webhook = await get_webhook(db, webhook_id)
    if db_webhook:
